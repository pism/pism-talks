...
\begin{frame}
  ...
  \note[item]{Antartica and, especially,
  \note[item]{the West Antarctic Ice Sheet have been dominating the news}
  \note[item]{today, I'd like to talk about the stability}
  \note[item]{of the Greenland Ice Sheet in the context of outlet glaciers}
...
\begin{frame}{Recent mass loss from Greenland}
  ...
  \note<1>[item]{While IPCC's Fifth Assessment contained projections from ice sheet models}
  \note<1>[item]{outlet glacier flow was basically abesent on those model representations}
...
\begin{frame}{Prognostic Simulations}
  \begin{block}{Planned}
    \vspace{2em}
    \begin{tikzpicture}[node distance = 2cm, auto]
      % Place nodes
      \node  (start) {};
      \node [glacial, node distance=4cm] (glacial) {LGM};
  ...
  \begin{block}{Today}
    ...
    \begin{tikzpicture}[node distance = 2cm, auto]
      % Place nodes
      \node  (start) {};
      \node [warm, node distance=4cm] (present day) {present day};
      \node [glacial, right of=present day, node distance=4cm] (glacial) {glacial};
      \node [hot, right of=glacial, node distance=4cm] (warm) {warm};
      % edges
...
\begin{frame}
  \begin{columns}
    \column[T]{5.5cm}
    \begin{figure}
      \movie[showcontrols=true,autostart,loop,width=5.5cm]{\includegraphics[width=5.5cm]{gris-glacial-3600m-img_0000}}{gris-glacial-3600m-hd1920.mov}
    \end{figure}
    \column[T]{6cm}
    \vspace{3em}
    \begin{figure}
