...
\begin{frame}
  ...
  \end{figure}
  \note<1>[item]{About two decades ago}
  \note<1>[item]{pioneers like R. Greve, C. Ritz, and P. Huybrechts}
  \note<1>[item]{developed the first ice sheet models to go after this question}
  \note<1>[item]{Here I'm going to show an example from Huybrechts}
  \note<1>[item]{You run a couple of warming scenarios}
  \note<2>[item]{and see what happens after a thousand years}
  \note<2>[item]{Well, if it gets warm enough, Greenland will become green\ldots}
...
\begin{frame}
  ...
  \begin{figure}
   \movie[showcontrols=true,autostart,loop,width=7.25cm]{\includegraphics[width=7.25cm]{../movies/gris_g900m_rcps_0000}}{../movies/gris-g900m_rcps-hd1920.mov}
  \end{figure}
\end{frame}
...
\begin{frame}
  \frametitle{Uncertainty analysis}
  \begin{itemize}
    \item uncertainties in ice dynamics dominate until 2200-ish
    \item then uncertainties in surface mass balanace take over
    \item uncertainties in ice-ocean interaction are very small
  \end{itemize}
\end{frame}
...
\begin{frame}[plain]
  \note[item]{In summary, Greenland will very likely become ice free within a millenniium }
